 ...
 * @param args
 */
case class Message(bundle: String, key: String, args: Any*)(implicit created: Position) {
   ...
   * @param methodname
   */
  def log(logger: Logger, level: Level, clsname: String, methodname: String)(implicit resolver: MessageResolver) = {
 ...
 *
 */
case class MessageResolver(locale: Locale = Locale.getDefault, loader: ClassLoader = null, control: Control = null) {

  private def rb( msg: Message) = {
    val useClassLoader = if (loader == null) getClass.getClassLoader else loader
    if (control == null) ResourceBundle.getBundle(msg.bundle, locale, useClassLoader)
   ...
   * @param methodname
   */
  def log(logger: Logger, level: Level, clsname: String, methodname: String, msg: Message) = {
...
object Message {
  val fsLog = Logger.getLogger(classOf[Message].getName(), null /* resource bundle */ );
   ...
   */
  def getNLSMessage(rb: ResourceBundle, bundleName: String, locale: Locale, key: String, args: Any*) =
    {
      try {
  ...
  def main(args: Array[String]) {
    if (args.length == 0) {
      System.out.println("Syntax: " + classOf[Message].getName() + " locale key [args]");
      System.out.println("Where:");
      System.out.println("  locale is the locale to use, default: \"-\", default locale");
      System.out.println("  key is a key in util.nls.messages.properties");
      System.out.println("  args are the arguments to message (only String args are supported)");
    ...
    implicit val resolver = if (locale==null) MessageResolver() else MessageResolver( locale = locale )
