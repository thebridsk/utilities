...
object ClassPath {
  ...
  def show( linePrefix: String = "", loader: ClassLoader = getClass.getClassLoader ) = {
    val b = showClasspath( new StringBuilder, linePrefix, loader )
    b.append(linePrefix).append("System properties:").append(fsCRLF)
    sys.props.foreach { e =>
      b.append(linePrefix).append("  ").append(e._1).append("=").append(e._2).append(fsCRLF)
    }
    b.toString()
  }

  @tailrec
  private def showClasspath( b: StringBuilder, indent: String, loader: ClassLoader ): StringBuilder = {
