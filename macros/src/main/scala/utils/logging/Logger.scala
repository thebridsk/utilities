...
//      println("Creating logger with name "+tag.runtimeClass.getName)
  ...
  def create( name: String, resource: String = null) : Logger = new Logger( name, resource )
...
class Logger private (name: String, resource: String) {
  ...
  def log(level: java.util.logging.Level, message: String): Unit = macro LoggerMacro.logMessage
  def log(level: java.util.logging.Level, message: String, cause: Throwable): Unit = macro LoggerMacro.logMessageCause
  def log(level: java.util.logging.Level, message: String, args: Any*): Unit = macro LoggerMacro.logMessageArgs
...
class LoggerMacro(override val c: Context) extends Source(c) {
  ...
  def logMessage(level : c.Expr[java.util.logging.Level], message: c.Expr[String]) = {
    import c.universe._
  ...
  def logMessageCause(level : c.Expr[java.util.logging.Level], message: c.Expr[String], cause: c.Expr[Throwable]) = {
    import c.universe._
    val logger = q"${c.prefix}.logger"
    val cls = getFullClassName(getClassSymbol)
    val method = getMethodName(getMethodSymbol)
  ...
  def logMessageArgs(level : c.Expr[java.util.logging.Level], message: c.Expr[String], args: c.Expr[Any]*) = {
    import c.universe._
    val logger = q"${c.prefix}.logger"
    val cls = getFullClassName(getClassSymbol)
    val method = getMethodName(getMethodSymbol)
  ...
  def warningMessageArgs( message: c.Expr[String], args: c.Expr[Any]*) = {
    import c.universe._
    logMessageArgs(c.Expr(q"java.util.logging.Level.WARNING"), message, args:_*)
  }
  ...
  def enteringArgs( args: c.Expr[Any]*) = {
    ...
    val method = getMethodName(getMethodSymbol)
    if (args.length == 1) {
      q"""if ($logger.isLoggable(java.util.logging.Level.FINER)) $logger.entering( $cls, $method, ${args(0)} )"""
    } else {
