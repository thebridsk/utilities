 ...
 * @author werewolf
 */
class EatCloseOutputStream( out: OutputStream, name: String=null, eatClose: Boolean = false) extends FilterOutputStream(out)
{
...
class ConsoleHandler extends MyStreamHandler
{
     ...
     * The stdout stream
     */
    protected val fOut: OutputStream = new EatCloseOutputStream(System.out, "out", true);
    setOutputStream(fOut);

    override
    def publish( record: LogRecord) = synchronized {
