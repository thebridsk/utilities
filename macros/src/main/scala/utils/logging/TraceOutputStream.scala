...
class TraceOutputStream( level: Level = Level.INFO,
                         loggername: String = classOf[TraceWriter].getName()
                       ) extends FilterOutputStream( new ByteArrayOutputStream )
{
  import TraceWriter._
    private val fLog: JLogger = JLogger.getLogger( loggername, null /* resource bundle */ )

    override
    def write(b: Array[Byte] ): Unit =  synchronized {
        if (!isFromLogging())
        {
    ...
    override
    def write(b: Array[Byte], off: Int, len: Int): Unit = synchronized {
        if (!isFromLogging())
        {
            out.write(b, off, len);
            var foundcrlf = false;
