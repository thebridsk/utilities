...
private object MyFormatterDefaults {
   ...
   * @param type
   */
  case class MessageType(val level: Level, logString: String, msgtype: MsgType = Normal) {
  ...
  def getTraceSpec(): String =
    {
      try {
        ...
        "Current trace specification: " + sepChar + "  " + new Config().getTraceSpec(sepChar + "  ");
      } catch {
  ...
  def getHeaderString(v: String): String =
    {
      try {
        ...
        builder.append("************ Start Display Current Environment ************").append(sepChar);
        ...
        builder.append("Java VM name = ").append(System.getProperty("java.vm.name")).append(sepChar);
        ...
        Config.getprogramClassLoader() match {
          case Some(l) =>
            builder.append("Classpath = ").append(sepChar); // .append(System.getProperty("java.class.path") ).append(sepChar);
        ...
        builder.append("************ End Display Current Environment ************").append(sepChar);
 ...
 * @author werewolf
 */
class MyFormatter(dateFmtDef: String,
    ...
    useLevelDef: Boolean = fsDefUseLevel) extends Formatter {
  ...
  private val dateFmt: DateFormat = new SimpleDateFormat(getProp(fsPropDateFmt, dateFmtDef))
  ...
  private val useFakeDate: Boolean = getPropBoolean(fsPropFakeDate, useFakeDateDef);

  private var fmtMsgOrNot: Boolean = getPropBoolean(fsPropFmtMsg, fsDefFmtMsg)

  private val useResource: Boolean = getPropBoolean(fsPropUseResource, useResourceDef)
  ...
  private val useMethodName: Boolean = getPropBoolean(fsPropUseMethodName, useMethodNameDef)
  ...
  private def getLinePrefix(record: LogRecord): String =
    {
      ...
      val threadStr =
        if (useThreadName) {
          ...
          String.format(threadFmt, Thread.currentThread().getId().asInstanceOf[Object]);
        }
  ...
  def format(record: LogRecord): String = synchronized {
    ...
    val mt = if (compact) formatPrefixCompact(outSB, record) else formatPrefixNotCompact(outSB, record)
  ...
  private def formatPrefixCompact(outSB: StringBuilder, record: LogRecord): MessageType =
    {
  ...
  private def formatPrefixNotCompact(outSB: StringBuilder, record: LogRecord): MessageType =
    {
  ...
  /*
     ...
     */
  override
  def formatMessage(record: LogRecord) = synchronized {
          ...
          if (fsUseMessageFormat) {
            ...
            mt.msgtype match {
              ...
              case Entry =>
                ...
                b.append("ENTRY")
                if (parameters != null) b.append(parameters.map { p => " %s" }.mkString(""))
                f = b.toString()
