...
class FileHandler( pattern: String = null ) extends StreamHandler {
    ...
    private def configure( pattern: String ) =
    {
        ...
        setFormatter(getClassObjectProperty(classOf[Formatter], cname + ".formatter", new FileFormatter()));
        try {
    ...
    private def getClassObjectProperty[T]( cls: Class[T], key: String, default: T ): T =
    {
        def value = getProperty(key);
        if (value != null)
        {
    ...
    private def rename() =
    {
        ...
        try
        {
          ...
          case e: Exception =>
            // We don't want to throw an exception here, but we
            // report the exception to any registered ErrorManager.
            reportError("Unable to rename "+fCurrentFile+" to "+fn, e, ErrorManager.GENERIC_FAILURE);
        }
    }
