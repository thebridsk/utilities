...
{
    private val fsLog = JLogger.getLogger( getClass.getName(), null /* resource bundle */)
    ...
    def traceStandardOutAndErr( stdout: Level = null, stderr: Level = null )
    {
        ...
        try
        {
            fsTraceOut = new TraceOutputStream(getLevel(stdout,TraceLevel.STDOUT), getClass.getName()+".StdOut");
            System.setOut(new PrintStream(new TeeOutputStream(fsOldStdOut, fsTraceOut)));
        } catch {
          case e: Exception =>
        ...
        try
        {
            fsTraceErr = new TraceOutputStream(getLevel(stderr,TraceLevel.STDERR), getClass.getName()+".StdErr");
            System.setErr(new PrintStream(new TeeOutputStream(fsOldStdErr, fsTraceErr)));
        } catch {
          case e: Exception =>
            if (fsLog.isLoggable(Level.FINE))
            {
