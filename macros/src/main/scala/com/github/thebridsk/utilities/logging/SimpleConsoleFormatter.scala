...
/**
  ...
  *   addException=<enum>
  *               how to show exception in trace message, valid values(default: short):
...
class SimpleConsoleFormatter(
    ...
    // 8 - short classname (String)
    // 9 - short loggername (String)
    defFormat: String = "%7$s",
    defFakeDate: Boolean = true,
    defFmtMsg: Boolean = true,
    defUseResource: Boolean = true,
    defShowKey: Boolean = false,
    defAddHeader: Boolean = false,
    defUseThreadName: Boolean = false,
    defAddException: String = "short"
