...
trait Logging {
  ...
  lazy val logger: Logger = Logger(getClass().getName, resource)
}
...
class Logger private (name: String, resource: String) {
  val logger: JavaLogger = JavaLogger.getLogger(name, resource)

  import scala.language.experimental.macros

  def getLevel = logger.getLevel
  def setLevel(level: java.util.logging.Level): Unit = logger.setLevel(level)
...
class LoggerMacro(override val c: Context) extends Source(c) {
  ...
  def throwingArg(arg: c.Expr[Throwable]): c.universe.If = {
    import c.universe._
    val logger = q"${c.prefix}.logger"
    val cls = getFullClassName(getClassSymbol)
    val method = getMethodName(getMethodSymbol)
