...
class ConfigArguments(val cmdline: ScallopConf, val defaultLevel: Option[Level] = None) {
  ...
  import cmdline._
  val logfile = opt[String]("logfile", noshort=true, descr="log filename pattern", argName="filename", default=None)
  val logconsolelevel = opt[Level]("logconsolelevel", noshort=true, descr="The level to use for console logging", argName="level", default=defaultLevel)

  def execute() = {
    logfile.foreach { filename => Config.setLogFile(filename) }
    logconsolelevel.foreach { level => Config.setLevelOnConsoleHandler(level) }
  }
