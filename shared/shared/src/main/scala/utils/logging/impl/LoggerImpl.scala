...
trait LoggerImplFactory extends LoggerFactory {
  ...
  def init( rootHandler: Handler*) = {
    if (LoggerFactory.factory != null && LoggerFactory.factory != this) {
      ...
      if (LoggerFactory.factory.isInstanceOf[LoggerImplFactory]) {
        throw new IllegalStateException("Replacement Logger.factory must extend LoggerImplFactory")
      }
    }
  ...
  private def getLoggerPrivate( name: String, parent: Option[Logger], resource: String ) = {
  ...
  private val rootLogger: LoggerImpl = {
    if (doingReplacement) LoggerFactory.factory.asInstanceOf[LoggerImplFactory].rootLogger
    else {
...
class LoggerImpl( name: String, parent: Option[Logger], var resource: String = null) extends Logger(name,parent) {
